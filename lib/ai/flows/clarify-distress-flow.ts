...
const ClarifyInputSchema = z.object({
    originalMessage: z.string().describe("The sanitized original message that triggered the self-harm signal."),
    studentClarification: z.string().describe("The student's response to Aura's gentle check-in question."),
});

const ClarifyOutputSchema = z.object({
    isGenuineDistress: z.boolean().describe("Whether this represents genuine distress requiring support. Be conservative — if uncertain, set to true."),
    summary: z.string().describe("One neutral sentence describing the context for a wellbeing report. No names. No quotes."),
});
