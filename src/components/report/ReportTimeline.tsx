...
import { CheckCircle2, User, MessageSquare, Edit, ShieldQuestion, Briefcase, Users, Info } from "lucide-react";
...
const ActorIcon = ({ actor }: { actor: TimelineEvent['actor'] }) => {
  switch (actor) {
    ...
    case 'System': return <ShieldQuestion className="h-4 w-4" />;
    default: return <Info className="h-4 w-4" />; // Fallback icon
  }
};
...
export default function ReportTimeline({ events }: ReportTimelineProps) {
  ...
  return (
    <ScrollArea className="w-full whitespace-nowrap pb-4">
      <div className="relative flex items-center py-8 min-w-max"> {/* Increased py for summary text space */}
        ...
        <div className="relative flex justify-between w-full px-2">
          {sortedEvents.map((event) => (
            <div key={event.id} className="relative flex justify-center"> {/* Wrapper for each event to position summary */}
              <Popover>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-semibold text-primary flex items-center">
                        <ActorIcon actor={event.actor} /> <span className="ml-2">{event.actor}</span>
                      </p>
                    ...
                    {event.details && (
                      <p className="text-xs text-muted-foreground bg-muted/50 p-2 rounded-md">{event.details}</p>
                    )}
                    {event.statusChange && (
                      <p className="text-xs">
                        Status: <Badge variant="outline">{event.statusChange.from || 'Initial'}</Badge>
                        <span className="mx-1">→</span>
                        <Badge variant={event.statusChange.to === 'Resolved' || event.statusChange.to === 'Closed' ? 'default' : 'secondary'}>
                    ...
                    {event.noteContent && (
                      <div className="mt-1">
                        ...
                        <p className="text-xs italic bg-secondary/30 p-2 rounded-md whitespace-pre-wrap">"{event.noteContent}"</p>
                      </div>
                    )}
                  </div>
                </PopoverContent>
