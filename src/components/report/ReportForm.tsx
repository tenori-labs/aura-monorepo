...
import {
  ...
  type ClassifyIncidentReportOutput,
} from '@/ai/flows/classify-incident-report';
...
export default function ReportForm() {
  ...
  const [classificationResult, setClassificationResult] =
    useState<ClassifyIncidentReportOutput | null>(null);
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Please be factual and objective. This information will be used to classify your
                report.
              </FormDescription>
              <FormMessage />
            </FormItem>
            ...
            <FormItem>
              ...
              <FormControl>
                <div className="flex flex-col items-center justify-center w-full">
                  <label
                    ...
                    className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer bg-muted/30 hover:bg-muted/50 border-muted-foreground/30"
                  >
                  ...
                  <div className="relative w-full max-w-sm p-2 border rounded-lg bg-muted/20">
                    ...
                    <div className="ml-4 flex-1">
                      <p className="text-sm font-medium truncate">
                        {form.getValues('multimediaFile')?.name}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {(form.getValues('multimediaFile')?.size / (1024 * 1024)).toFixed(2)} MB
                      </p>
                    </div>
                    <Button
                      variant="ghost"
              ...
              {fileType?.startsWith('video/') && (
                <FormDescription className="text-amber-600 dark:text-amber-500">
                  Note: AI classification currently only supports image analysis, not video content.
                </FormDescription>
              )}
              <FormMessage />
            </FormItem>
