...
export default function AiClassifierResult({ result }: AiClassifierResultProps) {
  return (
    <Card className="bg-secondary/50 border-accent shadow-md">
      <CardHeader>
        <CardTitle className="flex items-center text-md text-foreground"> {/* Changed text-accent-foreground to text-foreground */}
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center">
          ...
          <Badge variant="default" className="ml-2 bg-accent text-accent-foreground">{result.category}</Badge>
        </div>
        <div className="flex items-center">
          <CheckCircle className="mr-2 h-4 w-4 text-accent" />
          <span className="font-medium">Confidence:</span>
          <span className="ml-2 font-semibold text-accent">{(result.confidence * 100).toFixed(0)}%</span>
        </div>
