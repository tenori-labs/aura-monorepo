...
import { Clock, Tag, ListChecks, MessageSquare, AlertTriangle, HandCoins, Brush, CircleHelp } from "lucide-react";
...
export default function ReportCard({ report }: ReportCardProps) {
  ...
  const categoryIcon = report.aiClassification?.category ? getCategoryIcon(report.aiClassification.category) : getCategoryIcon(report.incidentType);
         ...
         {report.aiClassification && (
          <div className="mt-3 pt-3 border-t border-border">
            ...
            <div className="flex items-center gap-2 text-sm">
              {categoryIcon}
              <span>{report.aiClassification.category} (Confidence: {(report.aiClassification.confidence * 100).toFixed(0)}%)</span>
            </div>
            {report.aiClassification.keywords.length > 0 && (
              <div className="mt-1">
                <span className="text-xs text-muted-foreground">Keywords: </span>
                {report.aiClassification.keywords.map(kw => (
                  <Badge key={kw} variant="outline" className="mr-1 text-xs">{kw}</Badge>
                ))}
              </div>
            )}
          </div>
