...
import { UserCircle, BookOpen, Building, CheckSquare, GanttChartSquare, Loader2, Eye } from "lucide-react";
...
const generateDummyAssignedReports = (count: number, assignedToFaculty: string): Report[] => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    reports.push({
      ...
      assignedTo: isAssignedToThisFaculty ? assignedToFaculty : `Dr. Faculty ${String.fromCharCode(66 + (i % 5))}`, // Assign to current or others
      facultyNotes: isAssignedToThisFaculty && i % 2 === 0 ? `Initial review notes by ${assignedToFaculty}: Further investigation needed for item X.` : undefined,
    });
  }
      ...
      <Card className="shadow-lg">
        <CardHeader className="bg-muted/30">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <CardTitle className="text-3xl font-bold text-primary">{dummyFacultyMember.name}</CardTitle>
        ...
        <CardContent className="pt-6 grid md:grid-cols-2 gap-6">
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-foreground flex items-center"><UserCircle className="mr-2 h-5 w-5 text-primary" /> Personal Information</h3>
            <p className="text-sm"><strong className="text-muted-foreground">Email:</strong> {dummyFacultyMember.email}</p>
            <p className="text-sm"><strong className="text-muted-foreground">Office:</strong> {dummyFacultyMember.office}</p>
          </div>
          <div className="space-y-3">
            <h3 className="text-lg font-semibold text-foreground flex items-center"><Building className="mr-2 h-5 w-5 text-primary" /> Department</h3>
            <p className="text-sm"><strong className="text-muted-foreground">Name:</strong> {dummyFacultyMember.department}</p>
      ...
      <Card className="shadow-lg">
        ...
        <CardContent>
           <div className="border rounded-lg overflow-hidden">
            <Table>
              <TableCaption>
                {isLoading ? "Loading assigned reports..." : `A list of ${assignedReports.length} reports assigned to you.`}
              </TableCaption>
