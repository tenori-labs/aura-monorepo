...
const getDummyReportByIdForFaculty = (id: string): Report | undefined => {
  ...
  const facultyMembers = Array.from(
    { length: 10 },
    (_, i) => `Dr. Faculty ${String.fromCharCode(65 + i)}`
  );

  const randomIncidentType = incidentTypes[numericIdPart % incidentTypes.length];
  const randomStatus = reportStatuses[numericIdPart % reportStatuses.length];
  ...
  const initialFacultyNotes =
    numericIdPart % 4 === 0
      ? `Initial assessment by ${assignedTo}: Incident appears to be ${randomIncidentType.toLowerCase()}. Following up with involved parties.`
      : undefined;
  let lastUpdated = initialFacultyNotes
    ? addMinutes(dateReported, 30).toISOString()
    : dateReported.toISOString();
  ...
  const inReviewDate = addMinutes(
    parseISO(timelineEvents[timelineEvents.length - 1].date),
    5
  ).toISOString();
  timelineEvents.push({
    id: `evt-inreview-${id}`,
  ...
  if (randomStatus === 'Resolved' || randomStatus === 'Closed') {
    ...
    const actorForFinalStatus =
      randomStatus === 'Closed' && numericIdPart % 5 === 0 ? 'Admin' : 'Faculty';
    const finalActionBy = actorForFinalStatus === 'Admin' ? `Admin User` : assignedTo;
    const finalEventDate = initialFacultyNotes
      ? addMinutes(parseISO(timelineEvents[timelineEvents.length - 1].date), 30).toISOString()
      : addMinutes(parseISO(inReviewDate), 30).toISOString();
...
export default function FacultyReportDetailPage() {
  ...
  const handleStatusUpdate = async () => {
    if (!report || !selectedStatus) {
      toast({
        title: 'Update Failed',
        description: 'No report loaded or status selected.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (!facultyName) {
      toast({
        title: 'Update Failed',
        description: 'Report is not assigned to a faculty member. Cannot update.',
        variant: 'destructive',
      });
        ...
        <CardContent className="space-y-6">
          <div>
            ...
            <Select
              ...
              onValueChange={(value: string) => setSelectedStatus(value as Report['status'])}
            >
          ...
          <div>
            <Label htmlFor="faculty-notes" className="text-sm font-medium">
            ...
            <div className="mt-1 flex items-center justify-center w-full px-6 py-8 border-2 border-dashed rounded-md border-muted-foreground/50">
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  Describe any files in your notes for now.
                </p>
              </div>
            </div>
          </div>
