...
const getTypeIcon = (type: SelfHelpContent['type']) => {
    switch (type) {
        case "Video": return <PlayCircle className="h-10 w-10 text-white/80" />;
        case "Audio": return <Mic className="h-10 w-10 text-white/80" />;
        case "Article": return <FileText className="h-10 w-10 text-white/80" />;
        case "Image": return <Camera className="h-10 w-10 text-white/80" />;
        case "Interactive Guide": return <Compass className="h-10 w-10 text-white/80" />;
        default: return null;
    }
}
    ...
    return (
        <Card className="overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
            <CardHeader className="p-0 relative">
                ...
                {content.duration && <Badge variant="secondary" className="absolute bottom-2 right-2">{content.duration}</Badge>}
            </CardHeader>
...
export default function SelfHelpPage() {
    const categories: SelfHelpContent['category'][] = ["Mindfulness", "Stress Management", "Academic Success", "Healthy Habits", "Personal Safety & Awareness"];
    // Simulate a "For You" feed by mixing some content
    const forYouContent = [selfHelpContent[0], selfHelpContent[2], selfHelpContent[8], selfHelpContent[9]];
      ...
      <Card className="shadow-lg border-0 bg-gradient-to-r from-primary/10 to-blue-500/10 dark:from-primary/20 dark:to-blue-500/20">
        <CardHeader className="text-center p-8">
          ...
          <CardDescription className="mt-2 text-lg text-muted-foreground max-w-2xl mx-auto">
            Explore a curated collection of tools and resources to support your mental well-being and academic journey.
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Tabs defaultValue="foryou" className="w-full">
        <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 md:grid-cols-6">
          ...
          {categories.map(category => (
              <TabsTrigger key={category} value={category} className="capitalize">{category.replace(/ & /g, ' & ')}</TabsTrigger>
          ))}
        </TabsList>
        ...
        <TabsContent value="foryou" className="mt-6">
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {forYouContent.map(content => (
                    <Link key={content.id} href={`/self-help/${content.id}`} className="block h-full group">
        ...
        {categories.map(category => (
            <TabsContent key={category} value={category} className="mt-6">
                 <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {selfHelpContent.filter(c => c.category === category).map(content => (
                        <Link key={content.id} href={`/self-help/${content.id}`} className="block h-full group">
                             <ContentCard content={content} />
                        </Link>
                    ))}
