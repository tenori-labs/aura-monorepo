  ...
  const reportsBase = [
    {
      ...
      description:
        "My laptop was stolen from my bag when I stepped away for a moment. It's a silver MacBook Pro with a dent on the corner.",
    ...
    },
    {
      ...
      description:
        'A group of students made inappropriate comments and gestures towards me. This has happened multiple times.',
    ...
    },
    {
      ...
      description:
        'Graffiti was sprayed on the wall near the main entrance of the North Quad dorms. It appears to be gang-related.',
    ...
    },
    {
      ...
      description:
        'Broken fume hood, potentially leaking hazardous chemicals. Reported to lab technician but issue persists.',
          ...
          <div className="flex items-center gap-3 p-4 border rounded-lg bg-yellow-50 dark:bg-yellow-900/30 border-yellow-300 dark:border-yellow-700">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                <span className="font-medium">{bookedAppointment?.counselorName}</span> on{' '}
                <span className="font-medium">
                  {bookedAppointment &&
                    format(parseISO(bookedAppointment.appointmentDate), 'MMMM d, yyyy')}
                </span>{' '}
                at <span className="font-medium">{bookedAppointment?.appointmentTime}</span> is
        ...
        return (
          <div className="flex items-center gap-4 p-4 border rounded-lg bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-700">
            <Avatar className="h-16 w-16 border-2 border-green-500" data-ai-hint="counselor avatar">
              {bookedAppointment?.counselorAvatarUrl && (
                <AvatarImage
                  src={bookedAppointment.counselorAvatarUrl}
                  alt={bookedAppointment.counselorName}
                />
              )}
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {bookedAppointment?.counselorSpecialty}
              </p>
              <p className="text-sm text-muted-foreground">
                On{' '}
                <span className="font-medium">
                  {bookedAppointment &&
                    format(parseISO(bookedAppointment.appointmentDate), 'EEEE, MMMM d, yyyy')}
                </span>{' '}
                at <span className="font-medium">{bookedAppointment?.appointmentTime}</span>
              </p>
            </div>
          </div>
        );
          ...
          <div className="flex items-center gap-3 p-4 border rounded-lg bg-blue-50 dark:bg-blue-900/30 border-blue-300 dark:border-blue-700">
            ...
            <div>
              <p className="font-semibold text-blue-700 dark:text-blue-300">
                Appointment Status: {status.replace(/([A-Z])/g, ' $1').trim()}
              </p>
              <p className="text-sm text-muted-foreground">
                Your appointment with{' '}
                <span className="font-medium">{bookedAppointment?.counselorName}</span> on{' '}
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="pt-6 grid md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <p>
              <strong className="text-muted-foreground">Email:</strong> {dummyStudent.email}
            </p>
            <p>
              <strong className="text-muted-foreground">Major:</strong> {dummyStudent.major}
            </p>
          </div>
        ...
        <CardContent>
          {isLoadingAppointment ? (
            ...
            <div className="space-y-4">
              ...
              {(bookedAppointment.status === 'ConfirmedByCounselor' ||
                bookedAppointment.status === 'Upcoming') && (
              ...
              {bookedAppointment.status === 'PendingConfirmation' && (
                <p className="text-xs text-muted-foreground text-center">
                  You will be notified once your counselor confirms the appointment. You can also
