...
const dummyCounselorsData: Counselor[] = [
  {
    ...
    unavailableSlots: {
      // Slots Dr. Carter has blocked for herself
      [format(addDays(new Date(), 1), 'yyyy-MM-dd')]: [
        {
          id: `c1-unavailable-${format(addDays(new Date(), 1), 'yyyyMMdd')}-1300`,
          time: '01:00 PM',
        },
        {
          id: `c1-unavailable-${format(addDays(new Date(), 1), 'yyyyMMdd')}-1330`,
          time: '01:30 PM',
        },
      ],
      [format(addDays(new Date(), 2), 'yyyy-MM-dd')]: [
        {
          id: `c1-unavailable-${format(addDays(new Date(), 2), 'yyyyMMdd')}-1000`,
          time: '10:00 AM',
        },
      ],
    },
...
export const globalDummyAppointments: BookedAppointment[] = [
  // Alex Student - Current appointment - Pending
  {
    id: 'appt-1-alex-pending',
    studentId: 's123',
    ...
    auraChatSummary:
      'Alex engaged with Aura discussing exam stress and feelings of inadequacy. Aura noted increased anxiety markers and provided initial coping strategies. Student mentioned difficulty sleeping. No direct safety concerns expressed, but monitoring recommended.',
    counselorInstructionsForAura:
      'Check in with Alex about exam stress before our session. Remind about mindfulness exercises.',
  ...
  // Alex Student - Past completed appointment
  {
    ...
    status: 'Completed',
    bookingDate: subDays(new Date(), 8).toISOString(),
    counselorNotes:
      'Productive session. Alex is working on coping mechanisms for exam anxiety. Follow-up suggested in 2 weeks to check on progress with study habits.',
    auraObservationsForCounselor:
      'Session completed. Student reported feeling more prepared. Seems to be using mindfulness techniques discussed.',
    auraChatSummary:
      'Post-session: Aura had a brief check-in. Alex reported applying one technique and found it helpful.',
    auraRiskAssessment: 'No Risk',
  },
  // Alex Student - Past cancelled appointment
  {
    id: 'appt-alex-past-2-cancelled',
    studentId: 's123',
    studentName: 'Alex Student',
    counselorId: 'c1',
    counselorName: 'Dr. Emily Carter',
    counselorSpecialty: 'Stress & Anxiety Management',
    appointmentDate: format(subDays(new Date(), 14), 'yyyy-MM-dd'),
    appointmentTime: '11:00 AM',
    status: 'CancelledByStudent',
    bookingDate: subDays(new Date(), 15).toISOString(),
    counselorNotes:
      'Student cancelled due to unforeseen circumstances. Suggested rebooking when available.',
    auraRiskAssessment: 'No Risk',
  },
  {
    id: 'appt-2-jane-confirmed',
    studentId: 's456',
    ...
    status: 'ConfirmedByCounselor',
    bookingDate: subDays(new Date(), 1).toISOString(),
    counselorNotes: 'Discuss coping strategies for anxiety.',
    auraRiskAssessment: 'No Risk',
    auraChatSummary:
      'Jane used Aura for a brief check-in. Reported feeling generally well but sought tips for maintaining a positive outlook during a busy semester. Aura provided positive affirmations and resource links.',
    counselorInstructionsForAura:
      'Ask Jane if she found the positive affirmation exercises helpful.',
    meetingLink: `https://meet.example.com/session/appt-2-jane-confirmed-${Date.now().toString().slice(-5)}`,
  },
  {
    id: 'appt-sam-pending',
    studentId: 's009',
    studentName: 'Sam Pending',
    counselorId: 'c1',
    counselorName: 'Dr. Emily Carter',
    counselorSpecialty: 'Stress & Anxiety Management',
    appointmentDate: format(addDays(new Date(), 2), 'yyyy-MM-dd'),
  ...
  },
  {
    ...
    status: 'Completed',
    bookingDate: subDays(new Date(), 3).toISOString(),
    counselorNotes:
      'Good progress. Mike seems to be managing stress better by implementing a structured daily routine. Discussed time management strategies for upcoming project deadlines. He seemed receptive.',
    auraObservationsForCounselor:
      'Session completed. Student reported improvement in stress levels.',
  },
  {
    id: 'appt-chris-pending-high-risk',
    studentId: 's007',
    studentName: 'Chris P. Bacon',
    counselorId: 'c1',
    counselorName: 'Dr. Emily Carter',
    counselorSpecialty: 'Stress & Anxiety Management',
    appointmentDate: format(addDays(new Date(), 2), 'yyyy-MM-dd'),
    appointmentTime: '02:00 PM',
    status: 'PendingConfirmation',
    bookingDate: subDays(new Date(), 2).toISOString(),
    studentNotes: 'Need to discuss some personal issues urgently.',
    auraRiskAssessment: 'High Risk',
    auraChatSummary:
      "Chris's interaction with Aura was concerning. Student expressed feelings of hopelessness and isolation, and alluded to 'not wanting to be here anymore.' Aura immediately provided crisis hotline information (988) and strongly urged seeking professional help. This case requires urgent attention.",
    counselorInstructionsForAura:
      'URGENT: Monitor Chris for any further expressions of hopelessness. Ensure he is aware of crisis resources. Prioritize this appointment. If Aura senses escalation, alert campus security via established protocol.',
  },
  {
    id: 'appt-patty-confirmed',
    studentId: 's008',
    studentName: "Patty O'Furniture",
    counselorId: 'c1',
    counselorName: 'Dr. Emily Carter',
    counselorSpecialty: 'Stress & Anxiety Management',
    appointmentDate: format(addDays(new Date(), 3), 'yyyy-MM-dd'),
    appointmentTime: '11:00 AM',
    status: 'ConfirmedByCounselor',
    bookingDate: subDays(new Date(), 1).toISOString(),
    auraRiskAssessment: 'At Risk',
    auraChatSummary:
      'Patty discussed relationship difficulties with Aura. Expressed sadness and frustration. Aura listened empathetically and suggested resources for conflict resolution and emotional support.',
    meetingLink: `https://meet.example.com/session/appt-patty-confirmed-${Date.now().toString().slice(-5)}`,
  },
].sort(
  (a, b) =>
    parseISO(a.appointmentDate).getTime() - parseISO(b.appointmentDate).getTime() ||
...
export default function CounselorDashboardPage() {
  ...
  const [bookedAppointments, setBookedAppointments] = useState<BookedAppointment[]>(
    globalDummyAppointments.filter((app) => app.counselorId === loggedInCounselorId)
  );
  const [manuallyBlockedSlotsByDate, setManuallyBlockedSlotsByDate] = useState<
    Record<string, CounselorTimeSlot[]>
  >({});
  ...
  const [selectedAppointmentForInsights, setSelectedAppointmentForInsights] =
    useState<BookedAppointment | null>(null);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (foundCounselor) {
        ...
        setManuallyBlockedSlotsByDate(
          JSON.parse(JSON.stringify(foundCounselor.unavailableSlots || {}))
        );
        // Filter globalDummyAppointments for the logged-in counselor
        setBookedAppointments(
          globalDummyAppointments
            .filter((app) => app.counselorId === loggedInCounselorId)
            .sort(
              (a, b) =>
                parseISO(a.appointmentDate).getTime() - parseISO(b.appointmentDate).getTime() ||
                a.appointmentTime.localeCompare(b.appointmentTime)
  ...
  const handleToggleManualBlock = useCallback(
    (date: Date, slotToToggle: CounselorTimeSlot, block: boolean) => {
      ...
      setManuallyBlockedSlotsByDate((prevBlocked) => {
        ...
        if (block) {
          if (existingSlotIndex === -1) {
            ...
            toast({
              title: 'Slot Blocked',
              description: `Slot ${slotToToggle.time} on ${format(date, 'PPP')} is now manually blocked (Out of Office).`,
            });
          }
        } else {
          if (existingSlotIndex !== -1) {
            ...
            toast({
              ...
              description: `Slot ${slotToToggle.time} on ${format(date, 'PPP')} is now available (unless booked).`,
            });
          }
        }
  ...
  const handleAppointmentAction = (
    ...
    newStatus: BookedAppointment['status'],
    notes?: string
  ) => {
    const updatedAppointments = bookedAppointments.map((app) => {
      if (app.id === appointmentId) {
        const updatedApp = {
          ...app,
          status: newStatus,
          counselorNotes: notes || app.counselorNotes,
          bookingDate: new Date().toISOString(),
        };
    ...
    toast({
      ...
      description: `Appointment ${appointmentId} status changed to ${newStatus.replace(/([A-Z])/g, ' $1').trim()}.`,
    });
  ...
  const handleUpdateAuraInstructions = () => {
    ...
    const updatedAppointments = bookedAppointments.map((app) =>
      app.id === appointmentId
        ? { ...app, counselorInstructionsForAura: auraInstructionsInput }
        : app
    );
    setBookedAppointments(updatedAppointments);
    const globalIndex = globalDummyAppointments.findIndex((gApp) => gApp.id === appointmentId);
    if (globalIndex > -1)
      globalDummyAppointments[globalIndex].counselorInstructionsForAura = auraInstructionsInput;

    toast({
      title: 'Aura Instructions Updated',
      description: `Instructions for Aura regarding ${selectedAppointmentForInsights.studentName} have been saved.`,
    });
  };
  ...
  const getStatusBadgeVariant = (
    ...
    switch (status) {
      ...
      case 'Upcoming':
        return 'default';
      case 'RescheduleProposedByCounselor':
      case 'RescheduleProposedByStudent':
        return 'outline';
      ...
      .filter(
        (app) =>
          ...
          (isPast(parseISO(app.appointmentDate)) ||
            ['Completed', 'CancelledByStudent', 'CancelledByCounselor'].includes(app.status))
      )
      .sort(
        (a, b) => parseISO(b.appointmentDate).getTime() - parseISO(a.appointmentDate).getTime()
      );
  }, [bookedAppointments, selectedAppointmentForInsights]);

  const pendingAppointments = useMemo(() => {
    return bookedAppointments.filter(
      (app) => app.status === 'PendingConfirmation' && !isPast(parseISO(app.appointmentDate))
    );
  }, [bookedAppointments]);
    ...
    <div className="space-y-8 p-4 md:p-6 lg:p-8">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {counselor.email} | {counselor.office}
              </p>
            </div>
          </div>
        </CardHeader>
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Select a date to view your schedule (9 AM - 9 PM). Click available slots to mark them as
            Out of Office (manually blocked).
          </CardDescription>
        </CardHeader>
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            <div className="p-2 border rounded-md bg-background w-full md:w-auto">
              <Calendar
                ...
                className="shadow-sm"
                disabled={(date: Date) =>
                  isBefore(date, startOfDay(new Date())) && !isSameDay(date, startOfDay(new Date()))
                }
              />
              <Button
                variant="outline"
                ...
                onClick={() =>
                  toast({
                    ...
                    description:
                      'Full working hours configuration will be available in a future update.',
                  })
                }
              >
            ...
            <div className="flex-1 space-y-3 min-w-0 border rounded-lg p-4 bg-muted/10">
              <div className="flex justify-between items-center mb-3">
                <Button
                  ...
                  disabled={
                    isBefore(addDays(selectedDate, -1), startOfDay(new Date())) &&
                    !isSameDay(addDays(selectedDate, -1), startOfDay(new Date()))
                  }
                >
                ...
                <Button variant="outline" size="icon" onClick={() => handleDayNavigation(1)}>
              ...
              <ScrollArea className="h-[500px] border rounded-md p-1 bg-background">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-0.5">
                  {potentialSlotsForSelectedDay.map((slot) => {
                    const currentDateTimeForSlot = parseISO(
                      `${selectedDateString}T${format(parse(slot.time, 'hh:mm a', new Date()), 'HH:mm')}`
                    );
                    const isPastSlot =
                      isBefore(currentDateTimeForSlot, new Date()) &&
                      !isSameDay(selectedDate, new Date());

                    const appointmentInSlot = bookedAppointments.find(
                      (app) =>
                    ...
                    const isManuallyBlocked = manuallyBlockedForSelectedDay.some(
                      (bs) => bs.time === slot.time
                    );

                    let slotContent;
                    let slotClasses =
                      'p-2 text-xs text-left w-full h-[60px] flex flex-col items-start justify-center rounded-md';
                    let onClickAction = () => {};
                    let disabledInteraction = isPastSlot;

                    if (appointmentInSlot) {
                      slotClasses +=
                        ' bg-blue-100 dark:bg-blue-800/30 text-blue-700 dark:text-blue-300 border border-blue-300 dark:border-blue-700';
                      disabledInteraction = true;
                      slotContent = (
                        <>
                          <span className="font-semibold text-sm">{slot.time}</span>
                          <span className="text-xs truncate max-w-[90%] font-medium">
                            {appointmentInSlot.studentName}
                          </span>
                          <Badge
                            variant={getStatusBadgeVariant(appointmentInSlot.status)}
                            className="mt-0.5 text-[9px] px-1 py-0 leading-tight self-start"
                          >
                            {appointmentInSlot.status.replace(/([A-Z])/g, ' $1').trim()}
                          </Badge>
                          {appointmentInSlot.meetingLink &&
                            (appointmentInSlot.status === 'ConfirmedByCounselor' ||
                              appointmentInSlot.status === 'Upcoming') &&
                            !isPastSlot && (
                              <a
                                href={appointmentInSlot.meetingLink}
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-xs text-blue-600 hover:underline mt-0.5 flex items-center"
                          ...
                          {appointmentInSlot.status === 'PendingConfirmation' && !isPastSlot && (
                            <div className="flex gap-1 mt-1 self-start items-center">
                              <Button
                                variant="ghost"
                                size="sm"
                                className="h-5 px-1 py-0 text-green-600 hover:bg-green-100"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleAppointmentAction(
                                    appointmentInSlot.id,
                                    'ConfirmedByCounselor'
                                  );
                                }}
                              >
                                {' '}
                                <CheckSquare className="h-3 w-3 mr-1" /> Accept{' '}
                              </Button>
                              <Button
                                variant="ghost"
                                size="sm"
                                className="h-5 px-1 py-0 text-red-600 hover:bg-red-100"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleAppointmentAction(
                                    appointmentInSlot.id,
                      ...
                      slotClasses +=
                        ' bg-red-100 dark:bg-red-800/30 text-red-700 dark:text-red-300 border border-red-300 dark:border-red-700';
                      onClickAction = disabledInteraction
                        ? () => {}
                        : () => handleToggleManualBlock(selectedDate, slot, false);
                      slotContent = (
                        <>
                          <span className="font-semibold text-sm">{slot.time}</span>
                      ...
                      slotClasses +=
                        ' bg-green-50 dark:bg-green-800/20 hover:bg-green-100 dark:hover:bg-green-800/40 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-600';
                      onClickAction = disabledInteraction
                        ? () => {}
                        : () => handleToggleManualBlock(selectedDate, slot, true);
                      slotContent = (
                        <>
                          <span className="font-semibold text-sm">{slot.time}</span>
                          {!disabledInteraction && (
                            <CheckCircle className="h-3 w-3 mt-0.5 opacity-70 text-green-600" />
                          )}
                        </>
                      );
                    }
      ...
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Review and act on student appointment requests. Click student name for full insights and
            history.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {pendingAppointments.length > 0 ? (
                ...
                >
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div className="flex-grow">
                      <Link
                        href={`/counselor/student/${app.studentId}?appointmentId=${app.id}`}
                        passHref
                      >
                        <Button
                          variant="link"
                          className="p-0 h-auto text-lg font-semibold hover:underline text-primary"
                        >
                          {app.studentName}
                        </Button>
                      </Link>
                      <p className="text-sm text-muted-foreground">
                        Proposed: {format(parseISO(app.appointmentDate), 'PPP')} at{' '}
                        {app.appointmentTime}
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        Booked on: {format(parseISO(app.bookingDate), 'PPP p')}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2 items-center shrink-0 sm:ml-auto">
                      <Button
                        variant="default"
                        size="sm"
                        onClick={() => handleAppointmentAction(app.id, 'ConfirmedByCounselor')}
                        className="bg-green-600 hover:bg-green-700 text-white h-7 px-2 py-1"
                      >
            ...
            <p className="text-muted-foreground text-center py-8">
              No active appointment requests at this time.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {selectedAppointmentForModal && (
        <Dialog open={isRescheduleModalOpen} onOpenChange={setIsRescheduleModalOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                Propose Reschedule for {selectedAppointmentForModal.studentName}
              </DialogTitle>
              <DialogDescription>
                Current: {format(parseISO(selectedAppointmentForModal.appointmentDate), 'PPP')} at{' '}
                {selectedAppointmentForModal.appointmentTime}. Provide a reason for rescheduling.
                The student will be notified. (Full rescheduling to a new slot is a future feature).
              </DialogDescription>
            </DialogHeader>
              ...
              <Button
                onClick={() => {
                  if (selectedAppointmentForModal) {
                    handleAppointmentAction(
                      selectedAppointmentForModal.id,
              ...
              <div className="py-4 space-y-6 pr-4">
                <Card>
                  ...
                  <CardContent className="space-y-3 text-xs">
                    <div>
                      ...
                      <Badge
                        variant={getRiskAssessmentBadgeVariant(
                          selectedAppointmentForInsights.auraRiskAssessment
                        )}
                      >
                    ...
                    <div>
                      <h4 className="text-xs font-semibold text-muted-foreground mb-1">
                        Conversation Summary with Aura:
                      </h4>
                    ...
                    <div className="border-t pt-3 mt-3 space-y-2">
                      <h4 className="text-xs font-semibold text-muted-foreground mb-1">
                        Aura Post-Session Engagement Instructions:
                      </h4>
                      <Textarea
                        id="auraInstructions"
                      ...
                      />
                      <Button
                        onClick={handleUpdateAuraInstructions}
                        size="sm"
                        variant="outline"
                        className="text-xs"
                      >
                ...
                <Card>
                  ...
                  <CardContent className="text-xs">
                    {studentHistoryForModal.length > 0 ? (
                      <div className="space-y-2 max-h-40 overflow-y-auto">
                        {studentHistoryForModal.map((histApp) => (
                          <div key={histApp.id} className="p-2 border rounded-md bg-muted/40">
                            <p>
                              <strong>Date:</strong>{' '}
                              {format(parseISO(histApp.appointmentDate), 'PPP')} at{' '}
                              {histApp.appointmentTime}
                            </p>
                            <p>
                              <strong>Status:</strong>{' '}
                              <Badge
                                variant={getStatusBadgeVariant(histApp.status)}
                                className="text-[10px]"
                              >
                                {histApp.status.replace(/([A-Z])/g, ' $1').trim()}
                              </Badge>
                            </p>
                            {histApp.counselorNotes && histApp.status === 'Completed' && (
                              <p className="text-muted-foreground mt-1 text-[11px] italic">
                                Notes: {histApp.counselorNotes.substring(0, 100)}
                                {histApp.counselorNotes.length > 100 ? '...' : ''}
                              </p>
                            )}
                          </div>
                        ))}
                      ...
                      <p className="text-muted-foreground">
                        No prior appointment history found for this student.
                      </p>
                    )}
                  </CardContent>
                </Card>
                ...
                <div className="text-xs text-muted-foreground italic pt-4 border-t mt-4">
                  <p>
                    <strong>Reminder:</strong> This information is a supplementary tool. Summaries
                    and history aim to provide context while respecting student privacy.
                  </p>
                </div>
              </div>
            </ScrollArea>
